...
export function getMySQLPool(): mysql.Pool | null {
  ...
  try {
    ...
    console.warn('MySQL pool initialization skipped or failed:', err);
    return null;
  }
}

/**
 * Initializes MySQL tables if connected
 */
export async function initMySQLTables(): Promise<boolean> {
  ...
  try {
    ...
    console.warn('MySQL connection not currently active, using fallback persistence layer.');
    return false;
  }
}
